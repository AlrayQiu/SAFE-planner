...
template <class TMap>
class ParameterizedUrbsK6Opt final : public IOptimizeable{
public:
inline ParameterizedUrbsK6Opt(
...
inline void init_x(Eigen::VectorXd& x) const{
    ...
    for(int i = 0;i < urbs_.inner_points_count_;++i){
        x.segment<3>(i * 3) = (
            trajectory::UniformBSpline_K<6>::P0 *
            urbs_.Q_.block<6,3>(1 + i,0)).transpose();
    }
    x(t_index) = t_to_tau(urbs_.t_);
};
...
inline double prop_c_to_dt(const Eigen::MatrixX3d& G){
    const auto Q1 = urbs_.Q_.topRows(6);
    const auto Q2 = urbs_.Q_.bottomRows(6);

    const auto t1 = urbs_.t_;
    const auto t2 = t1 * t1; 
    const auto t3 = t2 * t1; 
    const Eigen::Matrix<double,6,3> MQ = (trajectory::UniformBSpline_K<6>::M * Q2);
    const Eigen::RowVector3d dv0 = -trajectory::UniformBSpline_K<6>::V0 / t2 * Q1;
    const Eigen::RowVector3d da0 = -2 * trajectory::UniformBSpline_K<6>::A0 / t3 * Q1;
    const Eigen::RowVector3d dpt = Eigen::RowVector<double,6>{0, -1, -2,-3,-4,-5} / t1 * MQ;
    const Eigen::RowVector3d dvt = Eigen::RowVector<double,6>{0, -1, -4,-9,-16,-25} / t2 * MQ;
    const Eigen::RowVector3d dat = Eigen::RowVector<double,6>{0, 0, -4, -18,-48,-100} / t3 * MQ;
...
UniformBSplineK6Opt<TMap> urbs_opt_;
const int t_index;
