...
class UniformBSplineK6Impl{
public:
inline UniformBSplineK6Impl(
    const std::vector<Eigen::Vector3d>& control_points, 
    const Eigen::Matrix<double,3,2>& head_va,
    const Eigen::Matrix<double,3,2>& tail_va)
    {
    ...
    inner_points_count_ = std::max<int>(points.size(),0);
    segment_count_ = inner_points_count_ + 1;
    control_points_count_ = inner_points_count_ + 6;

    
    M_.setZero(control_points_count_,control_points_count_);
    M_.block<1,6>(0,0) = UniformBSpline_K<6>::P0;
    for(int i = 0;i < inner_points_count_;i++){
        // M_(3 + i, i + 3) = 1;
        M_.block<1,6>(3 + i, i + 1) = UniformBSpline_K<6>::P0;
    }
    M_.block<1,6>(3 + inner_points_count_, inner_points_count_) = UniformBSpline_K<6>::Pt;
...
inline void build(UniformBSpline_K<6>& spline){
    spline.set_param(Q_, t_);
}
...
inline void set_param(const std::vector<Eigen::Vector3d>& inner_points,const double t){
    ...
    const auto t1 = t;
    const auto t2 = t1 * t1;
    M_.block<1,6>(1,0) = UniformBSpline_K<6>::V0 / t1;
    M_.block<1,6>(2,0) = UniformBSpline_K<6>::A0 / t2;
    M_.block<1,6>(4 + inner_points_count_, inner_points_count_) = UniformBSpline_K<6>::Vt / t1;
    M_.block<1,6>(5 + inner_points_count_, inner_points_count_) = UniformBSpline_K<6>::At / t2;
    lu_.compute(M_);
    Q_ = lu_.solve(Q_);
    // std::cerr << M_.fullPivLu().rank() << " " << M_.rows() << std::endl;
} 
