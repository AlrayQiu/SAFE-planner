...
template <typename PointType>
bool KD_TREE<PointType>::Nearest_Search(const PointType& point, PointType &Nearest_Points, double max_dist){
    MANUAL_HEAP q(2);
    q.clear();
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
        Search(Root_Node, 1, point, q, max_dist);
    }else {
        pthread_mutex_lock(&search_flag_mutex);
        while (search_mutex_counter == -1)
        {
    ...
    int k_found = min(1,int(q.size()));
    if(k_found == 0) return false;

    for (int i=0;i < k_found;i++){
        Nearest_Points = q.top().point;
        q.pop();
    }
    return true;
}
...
template <typename PointType>
void KD_TREE<PointType>::Add_Point(const PointType& PointToAdd){
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  
        Add_by_point(&Root_Node, PointToAdd, true, Root_Node->division_axis);   
    } else {
        Operation_Logger_Type operation{.point = PointToAdd,.op = ADD_POINT};
        pthread_mutex_lock(&working_flag_mutex);
...
int KD_TREE<PointType>::Add_Points(PointVector & PointToAdd, bool downsample_on){
    // size_t NewPointSize = PointToAdd.size();
    ...
    for (size_t i=0; i<PointToAdd.size();i++){
        if (downsample_switch){
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){  
                ...
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result)){
                    Operation_Logger_Type   operation_delete{.boxpoint = Box_of_Point,.op = DOWNSAMPLE_DELETE}, 
                                            operation{.point = downsample_result,.op = ADD_POINT};
                    pthread_mutex_lock(&working_flag_mutex);
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
                ...
                Operation_Logger_Type operation{.point = PointToAdd[i],.op = ADD_POINT};
                pthread_mutex_lock(&working_flag_mutex);
...
void KD_TREE<PointType>::Add_Point_Boxes(vector<BoxPointType> & BoxPoints){     
    for (size_t i=0;i < BoxPoints.size();i++){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
            ...
            Operation_Logger_Type operation{.boxpoint = BoxPoints[i],.op = ADD_BOX};
            pthread_mutex_lock(&working_flag_mutex);
...
void KD_TREE<PointType>::Delete_Points(PointVector & PointToDel){        
    for (size_t i=0;i<PointToDel.size();i++){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){               
            ...
            Operation_Logger_Type operation{.point = PointToDel[i],.op = DELETE_POINT};
            pthread_mutex_lock(&working_flag_mutex);        
...
int KD_TREE<PointType>::Delete_Point_Boxes(vector<BoxPointType> & BoxPoints){
    ...
    for (size_t i=0;i < BoxPoints.size();i++){ 
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){               
            ...
            Operation_Logger_Type operation{.boxpoint = BoxPoints[i], .op = DELETE_BOX};
            pthread_mutex_lock(&working_flag_mutex); 
...
void KD_TREE<PointType>::Delete_by_point(KD_TREE_NODE ** root, PointType point, bool allow_rebuild){   
    ...
    Operation_Logger_Type delete_log{.point = point, .op = DELETE_POINT};
    // struct timespec Timeout;    
...
void KD_TREE<PointType>::Add_by_range(KD_TREE_NODE ** root, BoxPointType boxpoint, bool allow_rebuild){
    ...
    Operation_Logger_Type add_box_log{.boxpoint = boxpoint,.op = ADD_BOX};
    // struct timespec Timeout;    
    if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr){
...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE ** root, PointType point, bool allow_rebuild, int father_axis){
    ...
    (*root)->working_flag = true;
    Operation_Logger_Type add_log{.point = point, .op = ADD_POINT};
    // struct timespec Timeout;    
    Push_Down(*root);
    if (((*root)->division_axis == 0 && point.x < (*root)->point.x) || ((*root)->division_axis == 1 && point.y < (*root)->point.y) || ((*root)->division_axis == 2 && point.z < (*root)->point.z)){
        if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr){          
...
template <typename PointType>
void KD_TREE<PointType>::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector & Storage, bool lock_handled ){
    if (root == nullptr) return;
    Push_Down(root);       
    if (boxpoint.vertex_max[0] <= root->node_range_x[0] || boxpoint.vertex_min[0] > root->node_range_x[1]) return;
    if (boxpoint.vertex_max[1] <= root->node_range_y[0] || boxpoint.vertex_min[1] > root->node_range_y[1]) return;
    ...
    if ((Rebuild_Ptr == nullptr) || root->left_son_ptr != *Rebuild_Ptr){
        Search_by_range(root->left_son_ptr, boxpoint, Storage, lock_handled);
    } else {
        if(!lock_handled)
            pthread_mutex_lock(&search_flag_mutex);
        Search_by_range(root->left_son_ptr, boxpoint, Storage, true);
        if(!lock_handled)
            pthread_mutex_unlock(&search_flag_mutex);
    }
    if ((Rebuild_Ptr == nullptr) || root->right_son_ptr != *Rebuild_Ptr){
        Search_by_range(root->right_son_ptr, boxpoint, Storage, lock_handled);
    } else {
        if(!lock_handled)
            pthread_mutex_lock(&search_flag_mutex);
        Search_by_range(root->right_son_ptr, boxpoint, Storage, true);
        if(!lock_handled)
            pthread_mutex_unlock(&search_flag_mutex);
    }
    return;    
...
template <typename PointType>
void KD_TREE<PointType>::Search_by_radius(KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage, bool lock_handled)
{
    if (root == nullptr)
        return;
    Push_Down(root);
    ...
    if ((Rebuild_Ptr == nullptr) || root->left_son_ptr != *Rebuild_Ptr)
    {
        Search_by_radius(root->left_son_ptr, point, radius, Storage,lock_handled);
    }
    else
    {
        if(!lock_handled)
            pthread_mutex_lock(&search_flag_mutex);
        Search_by_radius(root->left_son_ptr, point, radius, Storage, true);
        if(!lock_handled)
            pthread_mutex_unlock(&search_flag_mutex);
    }
    if ((Rebuild_Ptr == nullptr) || root->right_son_ptr != *Rebuild_Ptr)
    {
        Search_by_radius(root->right_son_ptr, point, radius, Storage, lock_handled);
    }
    else
    {
        if(!lock_handled)
            pthread_mutex_lock(&search_flag_mutex);
        Search_by_radius(root->right_son_ptr, point, radius, Storage, true);
        if(!lock_handled)
            pthread_mutex_unlock(&search_flag_mutex);
    }    
    return;
...
template class KD_TREE<safe_planner::planner::rrt_star::RRTNode>;